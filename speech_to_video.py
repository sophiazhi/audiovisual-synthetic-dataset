...
def parse_args():
    ...
    parser.add_argument('--video_corpus', help='Path to video corpus', default='/om2/user/szhi/corpora/childes_synthetic_video')
    parser.add_argument('--audio_corpus', help='Path to audio corpus', default='/om2/user/szhi/corpora/childes_synthetic_audio')
...
if __name__ == "__main__":
    ...
    for audio_path in tqdm(audio_paths):
        ...
        # audio_global_path = os.path.join(audio_corpus_path,audio_path) ## NEW
        audio = sf.SoundFile(input_audio_path) # audio = sf.SoundFile(audio_global_path) ## NEW
        target_length_sec = audio.frames / audio.samplerate
        
        make_input_video(base_video_tuple, target_length_sec, input_video_path)
        generate_deepfake(input_audio_path, input_video_path, output_video_path) ## NEW # generate_deepfake(audio_global_path, input_video_path, output_video_path)
